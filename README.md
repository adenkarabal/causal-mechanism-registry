...
[![License: CC BY 4.0](https://img.shields.io/badge/License-CC_BY_4.0-2f6f69.svg)](LICENSE.md)
...
The entries were elicited from the model that read the corpus, then tested for recurrence until only the structures the corpus would not stop producing remained. A human admits them.

A well-known name earns nothing here. `twin_crisis` is standard vocabulary in the crisis literature and it is not in the registry: existing entries already represent it without causal loss. Where an identifier shares a name with a term in the literature, that is convergence — two routes to the same structure.

The method is what generalises; the thirty-two finance-first entries are its first instance. [DISTILLATION.md](DISTILLATION.md) has the loop, the roles, and the scope.
...
└── LICENSE.md                human-readable licence notice
```

## From registry to signature

Scored for a particular moment, the registry becomes a compact world state: each entry with an intensity from 0 to 100, a direction, and a justified zero where the mechanism is absent. That vector is what carries historical comparison, analogue retrieval, negative-space analysis, dated public readings, and the conditioning of a language model without a long narrative.
...
**adenkarabal** leads the research and is responsible for the published judgments. **Claude by Anthropic** contributes synthesis, critique, and adversarial reading. **OpenAI Codex** contributes research architecture, verification, implementation, and editorial development. The working record is in [PROVENANCE.md](PROVENANCE.md).

## Criticism and reuse

Boundary challenges, prior art, counterexamples and failed applications go in the issues; the templates attach each objection to the mechanism or release it concerns. The boundaries in [MECHANISMS.md](MECHANISMS.md) are where the registry is most worth testing.

Citation metadata is in [CITATION.cff](CITATION.cff). The registry and documentation are available under [CC BY 4.0](LICENSE.md).
